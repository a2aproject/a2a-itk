document.addEventListener("DOMContentLoaded", () => {
    ...
    function getBorderIntersection(fromPos, toPos, isPairwise = false) {
        const dx = toPos.x - fromPos.x;
        const dy = toPos.y - fromPos.y;
        if (dx === 0 && dy === 0) return { x: fromPos.x, y: fromPos.y };

        const w = isPairwise ? 70 : 55; // half width of pill rectangle
        const h = isPairwise ? 17 : 14; // half height of pill rectangle
    ...
    function drawTopologyGraph(svgId, sdks, edges) {
        ...
        const isPairwise = sdks.length === 2;
        ...
        edges.forEach((edge) => {
            ...
            if (fromNode && toNode) {
                ...
                const startPt = getBorderIntersection(fromNode, toNode, isPairwise);
                const endPt = getBorderIntersection(toNode, fromNode, isPairwise);
        ...
        sdks.forEach((sdk, idx) => {
            ...
            const rectWidth = isPairwise ? 140 : 110;
            const rectHeight = isPairwise ? 34 : 28;
            ...
            text.style.fontSize = isPairwise ? "14px" : "11px";
